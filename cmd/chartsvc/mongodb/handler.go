...
//InitDBConfig sets MongoDB client and DB config for the ChartSVC API handler
...
func getPaginatedChartList(repo string, pageNumber, pageSize int, showDuplicates bool) (utils.ApiListResponse, interface{}, error) {
	...
	log.Debugf("Done. Returning %v charts.", len(charts))
	return newChartListResponse(charts), utils.Meta{TotalPages: totalPages}, nil
}
