...
func setupRoutes() http.Handler {
	...
	apiv1.Methods("GET").Path("/charts/search").Queries("match", "{match}").Handler(WithParams(findCharts))
	apiv1.Methods("GET").Path("/charts/{repo}").Handler(WithParams(listRepoCharts))
	apiv1.Methods("GET").Path("/charts/{repo}/search").Queries("match", "{match}").Handler(WithParams(findCharts))
