...
func Test_fetchAndImportFiles(t *testing.T) {
	...
	t.Run("file not found", func(t *testing.T) {
		...
		m.On("UpsertId", chartFilesID, common.ChartFiles{ID: chartFilesID, Readme: "", Values: "", Schema: "", Repo: charts[0].Repo, Digest: cv.Digest})
		dbSession := mockstore.NewMockSession(&m)
	...
	t.Run("authenticated request", func(t *testing.T) {
		...
		m.On("One", mock.Anything).Return(errors.New("return an error when checking if files already exists to force fetching"))
		chartFilesID := fmt.Sprintf("%s/%s-%s", charts[0].Repo.Name, charts[0].Name, cv.Version)
		m.On("UpsertId", chartFilesID, common.ChartFiles{ID: chartFilesID, Readme: testChartReadme, Values: testChartValues, Schema: testChartSchema, Repo: charts[0].Repo, Digest: cv.Digest})
		dbSession := mockstore.NewMockSession(&m)
		err := fetchAndImportFiles(dbSession, charts[0].Name, charts[0].Repo, cv)
		assert.NoErr(t, err)
		m.AssertExpectations(t)
...
func Test_emptyChartRepo(t *testing.T) {
	...
	m := mock.Mock{}
	m.On("One", &common.RepoCheck{}).Return(nil)
	dbSession := mockstore.NewMockSession(&m)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			m.On("One", repo).Run(func(args mock.Arguments) {
				*args.Get(0).(*common.RepoCheck) = tt.mockedLastCheck
			}).Return(nil)
			dbSession := mockstore.NewMockSession(&m)
